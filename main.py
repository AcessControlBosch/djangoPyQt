...
class Cadastros_menu(QMainWindow, Ui_cadastros_menu):
    def __init__(self):
        ...
        self.tabela.setColumnWidth(2, 500)
        self.tabela.setColumnWidth(3, 400)

        self.loaddata()

        self.tabela.setHorizontalHeaderLabels(["nome", "EDV", "JAburu", "Oi"])

    def loaddata(self):
        people = [{"NOME": "Dori", "EDV": "45", "JAburu": "NY", "Toma": "123"},
                  {"NOME": "Du", "EDV": "30", "JAburu": "HUMM", "Toma": "123"}]
        row = 0
        self.tabela.setRowCount(len(people))
        for person in people:
            self.tabela.setItem(row, 0, QTableWidgetItem(person["NOME"]))
            self.tabela.setItem(row, 1, QTableWidgetItem(person["EDV"]))
            self.tabela.setItem(row, 2, QTableWidgetItem(person["JAburu"]))
            self.tabela.setItem(row, 3, QTableWidgetItem(person["Toma"]))
            row = row + 1
    ...
    """def load_data(self):
        people = [{"Nome" : "Dorigon", "EDV" : "92896201"}, {"Nome" : "Duany", "EDV" : "92896202"}]
        row = 0
        self.tabela.setRowCount(len(people))

        for person in people:
            self.tabela.setItem(row,0, QtWidgets.QTableWidgetItem(person["Nome"]))
            self.tabela.setItem(row, 1, QTableWidgetItem(person["EDV"]))
            row = row+1"""
...
banco_dados = Connection("http://localhost:8000")
